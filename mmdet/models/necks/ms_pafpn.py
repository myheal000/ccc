...
class MSPAFPN(FPN):
    ...
    def forward(self, inputs):
        ...
        for i in range(0, used_backbone_levels - 1):
            if i==2:
                inter_outs[i + 1] += self.downsample_convs[i](self.downsample_convs[i-1](self.downsample_convs[i-2](inter_outs[i-2])))
                inter_outs[i] += self.downsample_convs[i-1](self.downsample_convs[i-2](inter_outs[i-2]))
        #这里相当于8被下采样，应该没有问题。
